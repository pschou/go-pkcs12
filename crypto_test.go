...
func TestPbDecrypt(t *testing.T) {
	for i, test := range pbDecryptTests {
		...
		plaintext, _, _, _, err := pbDecrypt(decryptable, password)
...
func TestPbEncrypt(t *testing.T) {
	...
	for i, c := range tests {
		...
		p, _ := bmpSliceZeroTerminated([]rune("sesame"))
